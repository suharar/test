...
# set PDF for all values 1, unified distribution
...
d <- dbeta(range, shape1 = 50, shape2 = 50)
...
# finding PDF based on prior = beta
...
no <- 1e3 #1000
set.seed(312)
# reproduce the prior with rbeta() with same assumptions
...
set.seed(123461)
# reproduce the posterior distribution with replacement
...
N <- 1e3
for (i in seq_along(prior_smp$smp)){ 
  # generates a single random binomial outcome
